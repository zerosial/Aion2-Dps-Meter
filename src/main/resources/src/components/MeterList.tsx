...
export const MeterList = memo(({ players, selectedId, onSelect, rowHeight }: Props) => {
  ...
  const sortedCache = useMemo(() => {
    ...
    return Array.from(cachedPlayersRef.current.values()).sort((a, b) => {
  ...
  return (
    <div className="grid gap-1">
      {sortedCache.map((cached) => {
        const isVisible = visibleIds.has(cached.id);
        const current = isVisible ? (rowMap.get(cached.id) ?? cached) : cached;
